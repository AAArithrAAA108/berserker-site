...
5. **Addendum (found while planning):** `image_index` was also the *only* thing determining a product's swatch display order (`fetchCatalog` orders colors by `image_index`, specifically to avoid a previously-fixed regression where ordering by row `id` showed swatches in random UUID order). Dropping it with nothing in its place would silently reintroduce that exact regression. `product_colors` gets a new `created_at timestamptz not null default now()` column (matching every other table in the schema) to take over as the ordering key — existing rows are backfilled with synthetic timestamps that preserve their current `image_index`-derived order exactly, so no product's swatch order changes when this ships; new colors naturally sort after all existing ones by real insertion time.
